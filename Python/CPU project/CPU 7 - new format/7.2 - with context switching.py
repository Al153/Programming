...
class Register:
    ...
    def write_high(self,value):
        ...
        self.value = (self.value^(value*256))&65535
...
class Vector_processing_unit:
    ...
    def run(self,data,op):
        ...
        # Coords
            ...
            x = self.Vector_accumulator[0].value&0xff
            y = self.Vector_accumulator[1].value&0xff
            self.write_vector_accumulator(int(sqrt(x**2+y**2)))
            ...
            self.write_vector_accumulator(int(sqrt((x-x1)**2+(y-y1)**2)))
...
class CPU:
    ...
    def run(self):
        ...
        if instr == 0: #halt
            ...
            self.Memory.write((self.Address_register.read()<<8)+parameter,self.Flags.value*256)
            ...
            self.Memory.write((self.Address_register.read()<<8)+parameter,self.Address_register.value*256)
                ...
                if skip:
                    ...
                    while skip_count:
                        ...
                        if to_test == 80:
                            ...
                            skip_count += 1
                        if self.PC.Addr1.value == 0 and self.PC.Addr2.value == 0:
                            out = (0,257) # halts because of no end if
...
start = time.time()
#print CPU.PC.Addr2.value,
output = CPU.run() #The first step is outside of the loop
#print CPU.Accumulator.value, "\t\t\t",CPU.Memory.memory[1:7]
while output[0] != 0:
    #print CPU.PC.Addr2.value,
    
    output = CPU.run()
    #print CPU.Accumulator.value , "\t\t\t",CPU.Memory.memory[1:7]#running of CPU
end = time.time()
print "Program ended"
print "exit code:",output[1]
print "Time taken:",end-start
print "Number of operations:", CPU.instr_count
print "Memory:\n",CPU.Memory.memory[:15]
