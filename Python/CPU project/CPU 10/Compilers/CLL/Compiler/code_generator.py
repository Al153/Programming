...
#		print self.global_variable_address_dict
		...
		self.assembly = optimise(self.assembly)
...
'''
	...
	push_pop_type_3 = '''Store gp0 Expression_stack [gp7]									#PUSH GP0
ADD gp7 @4
Compare gp7 stack_length
if Greater then Load PC Stack_overflow_error
...
'''
	push_pop_type_4 = '''Store gp0 Expression_stack [gp7]									#PUSH GP0
ADD gp7 @4
Compare gp7 stack_length
if Greater then Load PC Stack_overflow_error
...
'''
	...
	while push_pop_type_1 in assembly_code: #like type 0 but with an extra line
		#print "optimisations"
		index = assembly_code.index(push_pop_type_1)
		assembly_code = assembly_code[:index]+assembly_code[index+len(push_pop_type_1):]
	while push_pop_type_2 in assembly_code: #push gp0 pop gp1 ==> Move gp0 gp1
		#print "optimisations"
		index = assembly_code.index(push_pop_type_2)
		assembly_code = assembly_code[:index]+"Move gp0 gp1 						#PUSH gp0 POP gp1 "+assembly_code[index+len(push_pop_type_2):]	
	while push_pop_type_3 in assembly_code: #push gp0 pop index ==> Move gp0 index
		#print "optimisations"
		index = assembly_code.index(push_pop_type_3)
		assembly_code = assembly_code[:index]+"Move gp0 gp6 		 				#PUSH gp0 POP gp6 "+assembly_code[index+len(push_pop_type_3):]
	while push_pop_type_4 in assembly_code: #push gp0 pop index ==> Move gp0 index.  like type 4 but with an extra line
		#print "optimisations"
		index = assembly_code.index(push_pop_type_4)
		assembly_code = assembly_code[:index]+"Move gp0 gp6 		 				#PUSH gp0 POP gp6 "+assembly_code[index+len(push_pop_type_4):]
	return assembly_code
