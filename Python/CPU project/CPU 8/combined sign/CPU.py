...
class CPU:
	def __init__(self,program): #			accumulator								extended register 						index register 			
		self.Registers =  [register.Register(16,True,True,0,False),register.Register(16,True,True,0,False),register.Register(16,True,True,0,False)]
								#            Flags									 = zero									   = one
		self.Registers += [register.Register(16,False,True,0,True),register.Register(16,False,False,0,False),register.Register(16,False,False,1,False)]
								#			= all ones											jump register				
		self.Registers += [register.Register(16,False,False,0xffff,False),register.Register(16,True,True,0,False)]
								#			General purpose regs
		self.Registers += [register.Register(16,True,True,0,False),register.Register(16,True,True,0,False),register.Register(16,True,True,0,False),
							register.Register(16,True,True,0,False),register.Register(16,True,True,0,False),register.Register(16,True,True,0,False),
							register.Register(16,True,True,0,False),register.Register(16,True,True,0,False)]
	...
	def run(self):
		...
		while not self.halt:
			...
			#instr_string = self.instruction_list[opcode]
			#print "instruction being operated: ", instr_string
			#print "no. of instructions: \t\t\t", self.instr_count
			...
			if opcode == 0: #halt
				...
				if not load_data:
					...
					if load_abs:
						self.Registers[instr[0]].write(abs(data[1]),False,keep_dest_sign)
					else:
				...
				temp_pos = self.PC.pos #for storing
				if jump_data:
				...
				if compare_abs:
					...
					value2 = abs(value2)
				if wipe_flags:
					self.Registers[3].write((0),True,False) #clears flags

				if value1 == value2: #E
					self.Registers[3].write(1<<7,False,False)

				elif value1 > value2: #G
					self.Registers[3].write(1<<9,False,False)

				elif value1 < value2: #L
					self.Registers[3].write(1<<8,False,False)
