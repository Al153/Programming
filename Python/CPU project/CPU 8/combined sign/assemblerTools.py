...
def find_names(name):
    '''Finds the file and subroutine name from one string'''
    file_name = ''
    subroutine_name = ''
    name_is_file_name = 1
...
class Labellist: #allows for easy gotos and jumps
    ...
    def find_jump_offset(self,label,current_location): #used when a jump command is used
    ...
    def find_goto_location(self,label): #used when a goto command is used with a label
        print label
        label,offset = self.find_offset(label)
        print "label, offset: ", label,", ",offset
...
def assemble_type_r(opcode,line,line_number):
    ...
    word1 = (opcode<<8)+(register1<<4)+register2
    word2 = flag_value<<12
    return [word1,word2]
...
def assemble_type_a(opcode,line,line_number):
    ...
    word1 =(opcode<<8)+(register<<4)+flag_value
    if new_line[2][0] == "%":
...
def assemble_type_c(opcode,line,line_number):
    ...
    word1 = (opcode<<8)+(condition<<4)+flag_value
    word2 = address
    return [word1,word2]
