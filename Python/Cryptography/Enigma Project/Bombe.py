...
class Bombe:
        ...
        def run(self):
            ...
            for i in xrange(180):
                ...
                not_correct = [[[]]*len(self.ciphertexts[0])]*len(self.ciphertexts)
                
                for text in self.ciphertexts: #for each possible text
                    pos = self.ciphertexts.index(text)
                    print text
                    stop = 0
                    key = [0,0,0]
                    success = 1
                    #for k in xrange(len(text)):
                        #if text[k] in not_correct[pos][k]:
                            #stop = 1
                            #break
                    while not stop:
                        ...
                        for j in range(len(text)):
                            letter = text[j]
                            test = self.crib[j]
                            new_letter = self.Enigma.encrypt(letter)
                            #print new_letter
                            if new_letter != test:
                                #not_correct[pos][j].append(letter)
                                stop = 0
                                success = 0
                                key[2] = (key[2] + 1)%26
                                if key[2] == 0:
                    ...
                    if success: #if correct key found
                        position = self.crib_positions[self.ciphertexts.index(text)]
                        self.Enigma.change_rotor_keys(key[2],key[1],key[0])
