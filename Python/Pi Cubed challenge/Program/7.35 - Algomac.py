...
import math # for sqrt in VPU
#import Tkinter as tk

#Things such as N//2^x and N%2^x are replaced with things such as >> << &0xff... 


#Takes file input and feeds it to the CPU constructor
#Not actually part of the CPU, just interprets .txt file inputs
class Program:
    def __init__(self):
        error = 1
        while error:
...
class Stack:
    ...
    def push(self,value):
        ...
        if len(self.stack) == self.limit:
            del(self.stack[0])
            self.Flags.set_bits(2**3) #stack overflow flag
    def pop(self):
        stack_empty = 0
        try:
            out = self.stack.pop()
        ...
        if len(self.stack) == 0:
            self.Flags.set_bits(2**2) #Stack empty flag
        return out
