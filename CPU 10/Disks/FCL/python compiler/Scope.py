class Scope:
	...
	def byteHash(self,string):
		h = 0
		for char in string:
			h = hashTable[h ^ ord(char)]
		return h



def searchScopeListVariable(scopeList,varName):
	# ( Scope list * id -> Scope * type)
	if (scopeList.type == "Nil"):
		raise NameError()
	else:
		if varName in scopeList.head.var_dict:
			return (scopeList.head,var_dict[varName])
		else:
			return searchScopeListVariable(scopeList.tail,varName)

def searchScopeListType(scopeList,typeName):
	# ( Scope list * id -> Scope * struct )
	if (scopeList.type == "Nil"):
		raise NameError()
	else:
		if typeName in scopeList.head.var_dict:
			return (scopeList.head,var_dict[typeName])
		else:
			return searchScopeListType(scopeList.tail,typeName)

# hash table for the byteHash function
