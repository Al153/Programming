...
array CLL.array_of_MEMO 32 []
int CLL.MEMO CLL.array_of_MEMO
...
Move Stack_pointer previous_stack_ptr								%function:clearMemo
SUB Stack_pointer @12 									#OVERHEAD FOR FUNCTION clearMemo
Compare Stack_pointer Callstack_ptr
if Less then Load PC Recursion_limit_reached
Store ret_addr 0 [Stack_pointer]
Store previous_stack_ptr 4 [Stack_pointer]							#GETTING PARAMETERS FOR FUNCTION clearMemo
Load gp0 @0
Store gp0 Local.i [Stack_pointer] 						#STORE GP0
Pass 										%loopclearMemo-0entry 		#FOR LOOP
...
Move gp2 gp0
NOT gp0
if gp0 then Load PC loopclearMemo-0exit
Load gp0 @0
Load gp6 Local.i [Stack_pointer] 						#LOAD GP0
MUL gp6 @4 														   #INDEXING FOR @INT
ADD gp6 CLL.MEMO 											#STORE GP0 GLOBAL RELATIVE
Store gp0 0 [gp6]
Pass 										%loopclearMemo-0continue
Load gp1 Local.i [Stack_pointer] 						#LOAD GP0
Load gp0 @1
ADD gp0 gp1 														#ADD
Store gp0 Local.i [Stack_pointer] 						#STORE GP0
Load PC loopclearMemo-0entry
Pass 										%loopclearMemo-0exit
...
Load gp6 Local.x [Stack_pointer] 						#LOAD GP0
MUL gp6 @4 														   #INDEXING FOR @INT
ADD gp6 CLL.MEMO 											#LOAD GP0 GLOBAL RELATIVE
Load gp0 0 [gp6]									
if gp0 then Load gp0 @4294967295 									#COMPARE (IS TRUE)
NOT gp0 														    #IF STATEMENT
if gp0 then Load PC iffib-0endif
Load gp6 Local.x [Stack_pointer] 						#LOAD GP0
MUL gp6 @4 														   #INDEXING FOR @INT
ADD gp6 CLL.MEMO 											#LOAD GP0 GLOBAL RELATIVE
Load gp0 0 [gp6]									
...
OR gp0 gp1
if gp0 then Load PC iffib-1true 									#IF ELSE STATEMENT
...
if Greater then Load PC Stack_overflow_error
Goto function:print_i 												#CALLING print_i
Goto function:clearMemo 												#CALLING clearMemo
