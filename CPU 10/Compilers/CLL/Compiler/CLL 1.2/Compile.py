		...
		#				cont = raw_input("")
			...
			if parse_tree.children[0].type in ("<type_cast>","<term>","<factor>","<ternary_op>"):
				...
				try: 
					...
					print "ERROR: Could not find type of function: "+function_name
					quit()
				self.check_function_call(parse_tree.children[0])
	...
	def check_function_call(self,parse_tree):
		...
		if len(parse_tree.children) == 3:
			...
			if len(function_parameters) == 0:
				...
				print "ERROR(4): Function "+function_name.string+" takes "+str(len(function_parameters))+ " arguments, none given"
				quit()
		else:
