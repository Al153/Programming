...
public class LCSTopDownRecursive extends LCSFinder{
	...
	public int getLCSLength(){
		return fillTable(super.mString1.length()-1, super.mString2.length()-1); // call a recursive function
	}

	private int fillTable(int i, int j){
		Stack<int> priorityStack = new Stack<int>(); // to avoid another file, i,j are each pushed onto the stack


		while !(i>0 && j>0){
			priorityStack.push(j); // put the current values on the stack
			priorityStack.push(i);

			// then see what needs to be searched
			// set the coords to those values, then loop
