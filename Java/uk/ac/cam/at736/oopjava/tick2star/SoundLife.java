package uk.ac.cam.at736.oopjava.tick2star; 
...
public class SoundLife{
	...
	public static void print(boolean[][] world, AudioSequence as) { 
		//now adds sounds to a soundwave
   		for (int row = 0; row < world.length; row++) { 
      		for (int col = 0; col < world[row].length; col++) {
      			if (world[col][row]){
      				double freq = getFreq(row,col); //frequency derived from position
      				double volume = (1.0 + (double) countNeighbours(world, col, row))/9.0; //volume derived from number of neighbours
	...
	private static double getFreq(int row, int col){
		double magnitude = Math.floor(Math.sqrt((double)(row*row + col*col)));
		return 440 * Math.pow(2.0,(magnitude-49)/12.0); //picks a piano note
	...
	public static void play(boolean[][] world, int generations, java.io.OutputStream os) throws java.io.IOException {
	   AudioSequence as = new AudioSequence(0.5);
